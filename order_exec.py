...
# load trained model
model = keras.models.load_model("D:/mt5/LSTM_eurusd.model")
...
#msg = (datetime.now(), xhat, yhat, action)
msg = str(datetime.now()) +', '+ str(xhat) +', '+ str(yhat) +', '+ str(action)
with open("D:/mt5/log/log.txt","r+") as f:
    content = f.read()
    f.write(content + '\n' + msg)
